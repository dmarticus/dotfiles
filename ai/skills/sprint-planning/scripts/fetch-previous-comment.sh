#!/bin/bash
# Fetch the Feature Flags team's sprint planning comment from a given
# sprint issue.
#
# Searches issue comments for one containing "# Team Feature Flags".
...
# Match only "Feature Flags" to avoid picking up the Feature Flags
...
comment=$(gh api "repos/PostHog/posthog/issues/${issue_number}/comments?per_page=100" \
  --paginate \
  --jq '[.[] | select(.body | test("# Team Feature Flags"))] | first | .body // empty' \
  2>/dev/null) || comment=""
