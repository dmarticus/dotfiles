...
description: Audit the layered instruction stack (in-conversation user → ~/.claude/CLAUDE.md global → project CLAUDE.md → active skill → hooks/tool/system) for conflicting or ambiguous directives, and surface which layer should win. Use when the user says "check for instruction conflicts", "do my layers contradict", "audit my CLAUDE.md vs project/skill", "why is the agent ignoring X", or before relying on a deep skill stack.
---

<!-- Adapted from jasonv-skills/meta/instruction-conflicts (MIT, Jason Varbedian), built from ManyIH (arXiv:2604.09443) -->
...
2. **Global user** — `~/.claude/CLAUDE.md` (symlinked from `~/.dotfiles/ai/CLAUDE.md`) and `~/.claude/settings.json`.
3. **Project** — the repository's `CLAUDE.md`, `.claude/settings.json`, and any `.claude/` guidance.
4. **Skill** — active skill instructions (`~/.dotfiles/ai/skills/*/SKILL.md`) and agents (`~/.dotfiles/ai/agents/*.md`).
5. **Tool / harness / system** — tool defaults, hooks (markdownlint, ruff, lang-context), or harness behavior.

For each tier, record its source. If a source is unavailable, mark the layer absent; do not infer its directives.

More-specific instructions may override more-general ones within their scope. Treat explicitly pinned hard rules—such as security, money, secrets, or commit boundaries (e.g. "never post PR review comments without explicit user approval")—as non-overridable by lower tiers.
