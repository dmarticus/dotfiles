---
name: highsignal
description: 'Strip AI tells and filler from writing and rewrite in a plain human voice. Use when drafting or editing a tweet, thread, LinkedIn post, Slack update, email, PR description, or doc and you want high signal and no slop: lead with the real hook, cut throat-clears, value-teasers, manufactured quotability, parataxis, "it''s not just X, it''s Y", em dashes in posts, business-speak, vision-speak over concrete outcomes, and filler. Supports detect-only and rewrite modes. Trigger when the user says "clean this up", "make it sound human", "cut the slop", "tighten this post", or "does this read like AI".'
argument-hint: "[detect|rewrite] [<text or file>]"
---

<!-- Adapted from https://github.com/jpvarbed/highsignal (MIT, Jason Varbedian) -->
...
1. `tells` scans every sentence against the tells below and ships at 0 / Clean. Two checks
   come before calling it clean: count every em dash in the piece (see Em-dash overuse), and
   mark every run of two or more short period-separated sentences and check Parataxis.
...
published or sent under the author's name, runs the real fan-out instead: one subagent per
lens for pass 1, then the same verify-and-apply sweep for pass 2.
...
mode, ask the author for the one point, the reader, and the next action, then restructure
from the answers. In detect mode, flag "arc unclear" and list those three questions instead.
...
land harder. Don't flag ordinary informal writing.
...
dash, or spaced hyphens ` - ` standing in for an em dash. Do not skip the count because the
piece is "long-form" or otherwise fine.
...
## Where this plugs in locally

Other skills that draft public-facing text (`closing-time` Slack wrap-ups, `sprint-planning`
GitHub comments, PR descriptions) run the quick-edit pass — one reviewer, all four lenses —
before showing the user a draft. When invoked that way, stay in rewrite mode and don't list
the findings unless asked; just hand back the clean draft.
